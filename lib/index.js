...
AbstractProvider.prototype.create = function (docs, callback) {
    ...
    return this._getCollection(function (err, collection) {
        ...
        return collection.insertMany(docs, {w: 1}, tick(function (err, result) {
            if (err) return callback(err);
            if (!result || !result.ops || !result.ops.length) return callback(new Error('Не удалось создать документы: ' + JSON.stringify(docs)));
...
AbstractProvider.prototype.createOne = function (doc, callback) {
    ...
    return this._getCollection(function (err, collection) {
        ...
        return collection.insertOne(doc, {w: 1}, tick(function (err, result) {
            if (err) return callback(err);
            if (!result || !result.ops || !result.ops.length) return callback(new Error('Не удалось создать документ: ' + JSON.stringify(doc)));
...
AbstractProvider.prototype.update = function (conditions, update, callback) {
    ...
    return this._getCollection(function (err, collection) {
        ...
        return collection.updateMany(conditions, update, {w: 1, multi: true}, tick(function (err, result) {
            if (err) return callback(err);
            if (!result.modifiedCount) return callback(new Error('Не удалось обновить документы: ' + JSON.stringify(conditions)));
...
AbstractProvider.prototype.updateOne = function (conditions, update, options, callback) {
    ...
    return this._getCollection(function (err, collection) {
        ...
        // Обновляем документы
        return collection.updateOne(conditions, update, utils.extend({w: 1}, options), tick(function (err, result) {
            if (err) return callback(err);
            if (!result.modifiedCount) return callback(new Error('Не удалось обновить документ: ' + JSON.stringify(conditions)));
...
AbstractProvider.prototype.findAndUpdate = function (conditions, update, callback) {
    ...
    return this._getCollection(function (err, collection) {
        ...
        return collection.findAndModify(conditions, [], update, {new: true, w: 1}, tick(function (err, result) {
            if (err) return callback(err);
            if (!result || !result.value) return callback(new Error('Не удалось найти и обновить документ: ' + JSON.stringify(conditions)));
...
AbstractProvider.prototype.remove = function (conditions, callback) {
    ...
    return this._getCollection(function (err, collection) {
        ...
        // Удаляем документы
        return collection.removeMany(conditions, {w: 1}, tick(function (err, result) {
            if (err) return callback(err);
            if (!result.deletedCount) return callback(new Error('Не удалось удалить документы: ' + JSON.stringify(conditions)));
...
AbstractProvider.prototype.removeOnly = function (conditions, callback) {
    ...
    return this._getCollection(function (err, collection) {
        ...
        // Удаляем документы
        return collection.removeOne(conditions, {w: 1}, tick(function (err, result) {
            if (err) return callback(err);
...
AbstractProvider.prototype.findAndRemove = function (conditions, callback) {
    ...
    return this._getCollection(function (err, collection) {
        ...
        return collection.findAndRemove(conditions, [], {w: 1}, function (err, result) {
            if (err) return callback(err);
            if (!result || !result.value) return callback(new Error('Не удалось найти и удалить документ:' + JSON.stringify(conditions)));
